...
export default function Footer() {
  ...
  const footerLinks = [
    ...
    { label: t("linkProducts"), href: "#products" },
    { label: t("linkJourney"), href: "#journey" },
  ];
  ...
  return (
    <footer className="relative bg-forest-deep overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10 lg:px-14 py-16 md:py-22">
          ...
          >
            <div className="lg:col-span-7">
              ...
              >
                {t("ctaH3Line1")}
                <br className="hidden md:block" /> {t("ctaH3Line2Start")}{" "}
        ...
        >
          ...
          <div className="lg:col-span-4">
            <a href="#" className="inline-block mb-5">
              <img src="/logo-trans.png" alt="Hedone" className="h-28 w-auto brightness-0 invert" />
            </a>
          ...
          <div className="lg:col-span-2 lg:col-start-6">
            ...
            <ul className="space-y-3">
              {footerLinks.map((link) => (
                <li key={link.href}>
                  <a href={link.href} className="font-sans text-[0.85rem] text-cream/50 hover:text-cream transition-colors duration-300">
                    {link.label}
                  </a>
                </li>
              ))}
            ...
            <div className="space-y-5">
              <div>
                {contactInfo.phone.map((num) => (
                  <a key={num} href={`tel:${num.replace(/\s/g, "")}`} className="block font-sans text-[0.85rem] text-cream/50 hover:text-cream transition-colors duration-300 leading-relaxed">
                    {num}
                  </a>
                ))}
              </div>
              <div>
                {contactInfo.email.map((em) => (
                  <a key={em} href={`mailto:${em}`} className="block font-sans text-[0.85rem] text-cream/50 hover:text-cream transition-colors duration-300 leading-relaxed">
                    {em}
                  </a>
                ))}
              </div>
              <div>
                {contactInfo.address.map((line) => (
                  <p key={line} className="font-sans text-[0.8rem] text-cream/30 leading-relaxed">
                    {line}
                  </p>
                ))}
              </div>
      ...
      <div
        ...
        style={{
          backgroundImage: `radial-gradient(circle at 1px 1px, rgba(168,181,160,0.8) 1px, transparent 0)`,
